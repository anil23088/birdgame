...
public class catapult{
    ...
    public catapult(MyAngryBird game, OrthographicCamera cam) {
        this.game = game;
        ...
        startPoint1 = new Vector2(200 / ppm, 375 / ppm); // Left point
        endPoint1 = new Vector2(startPoint1);            // Start at same position

        startPoint2 = new Vector2(210 / ppm, 375 / ppm); // Right point
        endPoint2 = new Vector2(startPoint2);
    }

    public void render(float delta, Bird draggingBird) {
        game.getBatch().begin();
        game.getBatch().draw(catp, 200, 203); // Draw the catapult texture
        game.getBatch().end();
